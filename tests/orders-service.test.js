...
describe('orders service compatibility helpers', () => {
  it('validates status transitions like Flask', () => {
    ...
    expect(OrdersService.transitionError('delivered', 'pending')).toBe(
      'Transición de estado inválida: delivered -> pending',
    );
  });
  ...
  it('parses report dates with YYYY-MM-DD validation', () => {
    ...
    expect(OrdersService.parseReportDate('2024-02-30')).toEqual({ error: 'date debe usar formato YYYY-MM-DD' });
  });
